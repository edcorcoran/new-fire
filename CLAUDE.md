...
This repo departs from stock py4web in two ways that change these commands.

```bash
# Install (psycopg2-binary is only needed to read a local MusicBrainz mirror)
...
**`--app_names _default` is required.** `apps/_default` is a symlink to
`apps/newfire` that mounts the app at `/`, and py4web enumerates its apps folder
by listing it — so an unqualified `py4web run apps` imports the app twice as two
unrelated modules, each starting its own scheduler. See the README for why that
is worse than it sounds.

**Do not run `py4web setup apps`.** The stock apps (`_dashboard`,
`_documentation`, `_scaffold`, `_minimal`) have been removed from this repo on
purpose; `setup` reinstalls them from py4web's bundled copies. Plain `run` never
does.

Deployment is separate and documented in DEPLOY.md.
